...
class BPMF(object):
    ...
    def _update_average_features(self, iteration):
        self.avg_user_features_ *= iteration / (iteration + 1.0)
        self.avg_user_features_ += self.user_features_ / (iteration + 1.0)
        self.avg_item_features_ *= iteration / (iteration + 1.0)
        self.avg_item_features_ += self.item_features_ / (iteration + 1.0)
    ...
    def predict(self, data):
        ...
        u_features = self.avg_user_features_.take(
            data.take(0, axis=1).astype(int), axis=0
        )
        i_features = self.avg_item_features_.take(
            data.take(1, axis=1).astype(int), axis=0
        )
    ...
    def _update_item_params(self):
        ...
        # update mu_item
        mu_mean = (self.beta_item * self.mu0_item + N * X_bar) / (self.beta_item + N)
    ...
    def _udpate_item_features(self):
        ...
        for item_id in range(self.n_item):
            ...
            temp = self.beta * np.dot(features.T, rating) + np.dot(
