...
class YelpFLLoadAndPrepareDataset(luigi.Task):
    output_path: str = luigi.Parameter(default=os.path.join(os.getcwd(), "data/"))
    n_groups: int = luigi.IntParameter(default=4)

    def __init__(self, *args, **kwargs):
        super(YelpFLLoadAndPrepareDataset, self).__init__(*args, **kwargs)

        self.data_dir = os.path.join(self.output_path, "yelp_fl")
    ...
    def output(self):
        return {
            "items_df": luigi.LocalTarget(os.path.join(self.data_dir, "items_fl.csv")),
            "items_metadata": luigi.LocalTarget(
                os.path.join(self.data_dir, "items_metadata_fl.csv")
            ),
            "ratings_df": luigi.LocalTarget(
                os.path.join(self.data_dir, "review_fl.csv")
            ),
