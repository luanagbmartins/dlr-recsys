...
class YelpPDLoadAndPrepareDataset(luigi.Task):
    output_path: str = luigi.Parameter(default=os.path.join(os.getcwd(), "data/"))
    n_groups: int = luigi.IntParameter(default=4)

    def __init__(self, *args, **kwargs):
        super(YelpPDLoadAndPrepareDataset, self).__init__(*args, **kwargs)

        self.data_dir = os.path.join(self.output_path, "yelp_pd")
    ...
    def output(self):
        return {
            "items_df": luigi.LocalTarget(os.path.join(self.data_dir, "items_pd.csv")),
            "items_metadata": luigi.LocalTarget(
                os.path.join(self.data_dir, "items_metadata_pd.csv")
            ),
            "ratings_df": luigi.LocalTarget(
                os.path.join(self.data_dir, "review_pd.csv")
            ),
