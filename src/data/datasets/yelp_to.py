...
class YelpTOLoadAndPrepareDataset(luigi.Task):
    output_path: str = luigi.Parameter(default=os.path.join(os.getcwd(), "data/"))
    n_groups: int = luigi.IntParameter(default=4)

    def __init__(self, *args, **kwargs):
        super(YelpTOLoadAndPrepareDataset, self).__init__(*args, **kwargs)

        self.data_dir = os.path.join(self.output_path, "yelp_toronto")
    ...
    def output(self):
        return {
            ...
            "items_metadata": luigi.LocalTarget(
                os.path.join(self.data_dir, "items.csv")
            ),
